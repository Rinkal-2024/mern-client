      ...
      <nav className= {`py-4 lg:px-24 px-4 ${isSticky ? "sticky top-0 left-0 right-0 bg-blue-300 " : ""}`}>
        <div className="flex justify-between item-center text-base gap-8">
          ...
          <ul className="md:flex space-x-12 hidden">
                ...
                <Link
                    key={path} to={path} className="block text-base text-black uppercase curser-pointer hover:text-blue-700" >{link}
                </Link>
                ))}
          </ul>
        ...
        <div className= {`space-y-4 px-4 mt-16 py-7 bg-blue-700 ${isMenuOpen ? "block fixed top-0 left-0 right-0" :"hidden"}`}>
            {
                navItem.map(({ link, path }) => (
                    <Link
                      key={path} to={path} className="block text-base text-white uppercase curser-pointer hover:text-blue-700" >{link}
                    </Link>
                  ))
            }
        </div>
